...
(defn total
  []
  (->> @state/orders
       (map (fn [[id quan]] (* quan (get-in @state/gigs [id :price]))))
       (reduce +)))
...
(defn orders
  ...
  [:aside
   (if (empty? @state/orders)
     ...
     [:div.order
      [:div.body
     (for [[id quan] @state/orders]
       [:div.item {:key id}
        [:div.img
         [:img {:src (get-in @state/gigs [id :img])
                :alt (get-in @state/gigs [id :title])}]]
        [:div.content
         [:p.title (str (get-in @state/gigs [id :title]) " \u00D7 " quan)]]
        [:div.action
         [:div.price (format-price (* (get-in @state/gigs [id :price]) quan))]]
